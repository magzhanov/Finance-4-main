...
class CBSLSriLankaParser:
    ...
    def __init__(self, sleep_s: float = 0.2):
        ...
        self.source_url = "https://www.cbsl.gov.lk/en/press-releases/monetary-policy-review"
    ...
    def fetch_range(self, start_dt: datetime, end_dt: datetime, storage: LocalStorage) -> List[DocumentRecord]:
        ...
        blocks = soup.find_all("div", class_="view-content")
        container = blocks[-1] if blocks else soup
        ...
        for a in links:
            ...
            published_dt = self._parse_date_yyyymmdd(pdf_url) or self._parse_date_yyyymmdd(title)
            if not published_dt:
                continue
            ...
            rec = make_record(
                ...
                language="en",
                doc_type="Monetary Policy Review",
                text_path=text_path,                   
                pdf_urls=[pdf_url],
                pdf_paths=[pdf_path],
                meta={
                    "country": "Sri Lanka",
                    "source_name": "Central Bank of Sri Lanka",
